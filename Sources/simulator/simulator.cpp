...
void simulator::create_humans() {
  ...
  if (human::s_recover_time_min < 0 || human::s_recover_time_max < 0)
    throw std::invalid_argument("Recovery time is less than zero");
  if (human::s_recover_time_min > human::s_recover_time_max)
    throw std::invalid_argument("Recovery time min is bigger than max");

  human::s_base_ill_chance =
      ill_chance(m_config.get<double>("infection_chance"));
  if (m_config.get<double>("infection_chance") < 0)
    throw std::invalid_argument("infection_chance is less than zero");

  const long humans_ill = m_config.get<long>("humans_ill");
  const long humans_healthy = m_config.get<long>("humans_healthy");
  if (humans_ill < 2)
    throw std::invalid_argument("Number of ill humans is less than one");
  if (humans_healthy < 2)
    throw std::invalid_argument("Number of healthy humans is less than one");
...
void simulator::simulation_loop() {
  ...
  if (sim_time <= 0)
    throw std::invalid_argument("Simulation time is zero");

  const long infection_check = m_config.get<long>("infection_check");
  if (infection_check < 1)
    throw std::invalid_argument("Infection check time is less than 1");
  ...
  if (display_on_long != 0 && display_on_long != 1)
    throw std::invalid_argument("display_on can be only 1 or 0");
  bool display_on = display_on_long;
  long display_tick = m_config.get<long>("display_tick");
  if (display_tick < 1)
    throw std::invalid_argument("display_tick is less than 1");
